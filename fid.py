...
parser.add_argument('-s', '--num_samples', type=int, help="number of samples used for culcurating fid score", default=10000)
...
def fid(epochs, basedir, real_path, device, method, conditional, num_batch, batch_size, nz, node=0):
    fake_path = os.path.join(basedir, 'fake')
    fid_vals = []
...
def prepare_real(basedir, batch_size, num_batch, device):
    real_path = os.path.join(basedir, 'real')
...
def main():
    ...
    method = args.method
    batch_size = 256
    num_batch = num_sumples // batch_size + 1

    device = torch.device(f"cuda:{args.gpu_num}" if torch.cuda.is_available() else "cpu")

    basedir = 'tmp_' + str(datetime.now())
    real_path = prepare_real(basedir, batch_size, num_batch, device)

    epochs = list(range(0, epoch+1, 50 if method == "wafl" else 50 if method == "fl" else 10))
    fid_vals = fid(epochs, basedir, real_path, device, method, conditional, num_batch, batch_size, nz)
